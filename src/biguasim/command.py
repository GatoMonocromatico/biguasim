"""This module contains the classes used for formatting and sending commands to the BiguaSim
backend. Most of these commands are just used internally by BiguaSim, regular users do not need to
worry about these.
...
class Command:
    ...
    Commands are used for IPC between the biguasim python bindings and biguasim
    binaries.
...
class CommandCenter:
    ...
    Args:
        client (:class:`~biguasim.biguasimclient.BiguaSimClient`): Client to send commands to
    ...
    def __init__(self, client):
        ...
        # This is the size of the command buffer that BiguaSim expects/will read.
        # self.max_buffer = 10485760
        self.max_buffer = 1073741824
        # self.max_buffer = 2147483648
        # self.max_buffer = 1125899906842624
    ...
    def _write_to_command_buffer(self, to_write):
        ...
        to_write += '0'  # The gason JSON parser in biguasim expects a 0 at the end of the file.
...
class SpawnAgentCommand(Command):
    ...
    def set_location(self, location):
        ...
        """
        if len(location) != 3:
            raise BiguaSimException("Invalid location given to spawn agent command")
    ...
    def set_rotation(self, rotation):
        ...
        """
        if len(rotation) != 3:
            raise BiguaSimException("Invalid rotation given to spawn agent command")
...
class AddSensorCommand(Command):
    """Add a sensor to an agent
        ...
        Args:
            sensor_definition (~biguasim.sensors.SensorDefinition): Sensor to add
    """
