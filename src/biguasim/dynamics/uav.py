...
class QuadCopterX(VehicleModel):
    ...
    def _build_params(self, params : dict):
        ...
        # Max yaw error (rad) the decoupled cmd_pos_yaw governor acts on per tick, so large
        # heading commands are slewed through small, always-stable yaw errors.
        self.batched_params.yaw_slew_max = float(params.get('yaw_slew_max', 0.4))
    ...
    def get_cmd_motor_speeds(self, state : dict, control : dict):  #control needs to be a tensor
        ...
        if self.control_abstraction == 'cmd_pos_yaw':
            ...
            # Reference governor for the decoupled path: clamp the yaw error the controller
            # acts on so a large heading command is slewed through as a sequence of small,
            # always-stable yaw errors. yaw_sp is the (moving) intermediate heading setpoint.
            if bp.pos_yaw_decoupled:
                yaw_err_ctrl = torch.clamp(yaw_err, -bp.yaw_slew_max, bp.yaw_slew_max)
                yaw_sp = yaw + yaw_err_ctrl
            else:
                yaw_err_ctrl = yaw_err
                yaw_sp = yaw_des
            ...
            # -- Step 3: Select per batch element --
            # The decoupled (robust) path always uses the true current rotation R_q; the
            # R_r/yaw_des swap creates a discontinuity that destabilises large yaw slews.
            if bp.pos_yaw_decoupled:
                R = R_q
            else:
                R = torch.where(mask, R_r, R_q)
            ...
            _yaw_ref = yaw_sp if bp.pos_yaw_decoupled else yaw_err
            ...
            if bp.pos_yaw_decoupled:
                # Robust path: R_des already encodes the (governed) heading yaw_sp, so the
                # geometric attitude error handles most of the yaw. A small dedicated PD
                # term on the clamped error adds crisp, well-damped yaw tracking.
                M_yaw = bp.kp_yaw[idxs].squeeze(-1) * yaw_err_ctrl - bp.kd_yaw[idxs].squeeze(-1) * yaw_rate
            else:
                # Original path: R_des heading follows yaw_err, so yaw is driven by an
                # explicit PD term added to the yaw axis.
