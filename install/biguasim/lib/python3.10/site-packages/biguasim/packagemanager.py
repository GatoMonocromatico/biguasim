"""Package manager for worlds available to download and use for BiguaSim"""
import json
import os
import shutil
import sys
...
def available_packages():
    """Returns a list of package names available for the current version of BiguaSim

    Returns (:obj:`list` of :obj:`str`):
        List of package names
    """
    # Get the index json file from the backend
    url = "{ver}/available".format(ver=util.get_biguasim_version())
    try:
...
def world_info(world_name, world_config=None, base_indent=0):
    ...
    if world_config is None:
        raise BiguaSimException("Couldn't find world " + world_name)
...
def _find_file_in_worlds_dir(filename):
    """
    Recursively tries to find filename in the worlds directory of biguasim
    ...
    """
    for root, _, filenames in os.walk(util.get_biguasim_path(), "worlds"):
...
def install(package_name, url=None, branch=None, commit=None):
    ...
    if(_validVersion(user_version, min_version) == False):
        print("You must update BiguaSim to at least version 1.0.0 to continue. Packages are not available for earlier versions.")
        return 
    
    if package_name is None and url is None:
        raise BiguaSimException("You must specify the URL or a valid package name")
...
def _check_for_old_versions():
    ...
    path = util._get_biguasim_folder()
    ...
    for f in os.listdir(path):
        ...
        if f == "ignore_old_packages":
            return
        if f == util.get_biguasim_version():
            continue
    ...
    if not_matching:
        print("**********************************************")
        print("* You have old versions of BiguaSim packages *")
        print("**********************************************")
...
def prune():
    """Prunes old versions of biguasim, other than the running version.
    ...
    **DO NOT USE WITH HOLODECKPATH**
    ...
    holodeck_folder = util._get_biguasim_folder()
    ...
    for file in os.listdir(holodeck_folder):
        ...
        if os.path.isfile(file_path):
            continue
        if file == util.get_biguasim_version():
            continue
        # Delete it!
...
def get_scenario(scenario_name):
    ...
    if config_path == "":
        raise FileNotFoundError(
            ...
            "is installed.".format(file=scenario_name, path=util.get_biguasim_path()))
...
def get_package_config_for_scenario(scenario):
    ...
    raise BiguaSimException("Could not find a package that contains world {}".format(world_name))


def _iter_packages():
    path = util.get_biguasim_path()
