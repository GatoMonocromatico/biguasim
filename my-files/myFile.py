...
try:
    import cv2
except ImportError:  # display is optional; the depth data is read either way
    cv2 = None
    print("cv2 not installed — depth window disabled (pip install opencv-python)")

PROFILE = VEHICLE_REGISTRY["BlueROV2"]   # or BlueROVHeavy, DjiMatrice, TorpedoAUV, BlueBoat
TICKS_PER_SEC = 200

DEPTH_NAME = "DepthCamera"
DEPTH_RES = 256      # keep small: this buffer is copied every capture
DEPTH_HZ = 10        # must divide TICKS_PER_SEC evenly
DEPTH_MAX_M = 30.0   # display clip only
...
SKY_SENTINEL_M = 655.0   # sky pixels saturate UE's float16 depth buffer at 65504 cm


def show_depth(depth):
    """depth: (H, W) float32, metres from the camera plane."""
    finite = np.nan_to_num(depth, nan=DEPTH_MAX_M, posinf=DEPTH_MAX_M)
    finite[finite >= SKY_SENTINEL_M] = DEPTH_MAX_M
    if cv2 is None:
        h, w = finite.shape
        print(
            f"depth {finite.shape} "
            f"min={finite.min():.2f}m max={finite.max():.2f}m "
            f"centre={finite[h // 2, w // 2]:.2f}m"
        )
        return True
    gray = (finite.clip(0, DEPTH_MAX_M) / DEPTH_MAX_M * 255).astype(np.uint8)
    cv2.imshow("depth", cv2.applyColorMap(255 - gray, cv2.COLORMAP_TURBO))
...
with env:
    ...
    try:
        while True:
            ...
            if tick % DISPLAY_EVERY == 0 and DEPTH_NAME in agent_state:
                if not show_depth(agent_state[DEPTH_NAME]):
                    break
